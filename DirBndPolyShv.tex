...
\title{Dropping terms of the form $0^\yon$ in Dirichlet series\\is a truncation}

\author{David and David}

\maketitle
\begin{abstract}
One can think of polynomials such as $2\yon^3+\yon+5$ in one variable as functors from the category $\finset$ of finite sets to itself; call the category of polynomial functors and the natural transformations between them $\poly$. The constants $0,1$ and operations $+,\times$ are the initial and terminal objects and the coproduct and product in $\poly$. 

Just as the polynomial functors are the copresheaves that can be written as sums of representables, one can express Dirichlet series such as $2\cdot3^\yon + 1+ 5\cdot0^\yon$ and coproducts of representable presheaves. Note that one would usually omit $5\cdot 0^\yon$ as it is ``just 0'', but it seems to us this should be considered a truncation. Indeed, the category $\dir$ of Dirichlet functors has very nice formal properties---has all limits and its $0,1,+,\times$ agree with usual notation---and yet here the object $0^\yon$ is not $0$. It is the representable functor on the empty set, and we argue it is reasonable to \emph{not} drop them.

While our main purpose is to say that terms of the form $0^\yon$ should not be dropped unless there is computational power or some other reason to do so, we also prove some very basic theorems, e.g.\ that the category of Dirichlet series is equivalent to the category $\smset^{\to}$ of functions $\pi\colon s\to t$. Younger readers can be asked to determine which object of $\smset^\to$ corresponds to $0^\yon$ under this equivalence.
\end{abstract}
...
\]
There are of course infinite versions of the above, but in this paper we work entirely in the topos $\finset$ of finite sets, to keep things simple. If one wants infinite Dirichlet series or power series, one get them and much more by replacing $\finset$ with $\smset$ everywhere.
...
The categories $\poly$ and $\dir$ each has pullbacks and an idempotent monad, $\eta_P\colon P\mapsto P(1)$ and $\eta^D\colon D\mapsto D(0)$, and we can recover \cref{eqn.finite_sum} using pullbacks 
\end{remark}
