...
The Dirichlet monoidal structure is closed; that is, for any $P,Q:\poly$ we define
\begin{equation}\label{eqn.poly_mon_closed}
[P,Q]\coloneqq\prod_{i:P(1)}Q\circ(p_i\yon),
\end{equation}
for example $[n\yon,\yon]\cong \yon^n$ and $[\yon^n,\yon]\cong n\yon$.
\begin{proposition}
We have an adjunction 
\[
	\poly(A\otimes P,Q)\cong\poly(A,[P,Q]).
\]
\end{proposition}
\begin{proof}
We first note that $Q\circ p\yon=[\yon^p,Q]$ and that $\poly(\yon^n,[P,Q])\cong[P,Q](n)$ and $\cong\poly(P,Q)^n$.
\begin{align*}
	\poly(A\otimes P,Q)&\cong
	\poly\left(\sum_{i:A(1)}\sum_{j:P(1)}\yon^{a_i}\otimes\yon^{p_j},Q\right)\\&\cong
	\prod_{i:A(1)}\prod_{j:P(1)}\poly(\yon^{a_ip_j},Q)\\&\cong
	\prod_{i:A(1)}\prod_{j:P(1)}Q(a_ip_j)
\end{align*}
**finish**
\end{proof}
...
\begin{remark}
Let $s:\fin$ be a finite set; the functor part of the usual $s$-states monad on $\fin$ is $n\mapsto (sn)^s$, and the corresponding polynomial is $(s\yon)^s$. This is given by internal global sections of $s\yon^s$:
\[(s\yon)^s\cong[s\yon^s,\yon].\]
\end{remark}
