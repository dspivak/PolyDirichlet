...
Day convolution for the cartesian product monoidal structure provides another symmetric monoidal structure $\otimes\colon\poly\times\poly\to\poly$, for which the monoidal unit is $\yon$. We can write an explicit formula for $P\otimes Q$, with $P,Q$ as in \cref{eqn.finite_sum}:
\[
  P\otimes
  Q=
  \sum_{i=1}^{P(1)}\sum_{j=1}^{Q(1)}\yon^{p_iq_j}
\]
We call this the \emph{Dirichlet product} of polynomials, because on objects it coincides with the Cartesian product on Dirichlet polynomials via the Dirichlet transform (which we introduce in the next section):
\[\overline{P\otimes Q}=\overline{P}\times\overline{Q}.\]
Polynomials functors can also be composed $(P\circ Q)(n)\coloneqq P(Q(n))$; this gives a nonsymmetric monoidal structure on $\poly$.
