...
While we will not pursue it here, one can take $\poly_\smset$ to be the full subcategory of functors $\smset\to\smset$ spanned by small coproducts of representables, and similarly for $\dir_\smset$. Note that \cref{def.poly_dir} generalizes to any category, but it is not generally equivalent to the usual definition of polynomial functor, defined on any locally cartesian closed category.
...
Recall that a natural transformation is called \emph{cartesian} if its naturality squares are pullbacks.
...
Now that we have established the equivalences $\poly \simeq \cont$ and $\dir \simeq \bun$, it remains to check that they identify cartesian natural transformations with cartesian morphisms (in the sense of \cref{def.sheaves_bundles}). For polynomial functors, we may refer to \cite[Section 2]{GK:Polynomial.Functors}. We now turn to Dirichlet functors; we want to show that the square
\begin{equation}\label{eqn.cart1}
\begin{tikzcd}
	D(1)\ar[r, "f(1)"]\ar[d, "\pi"']&
	D'(1)\ar[d, "\pi'"]\\
	D(0)\ar[r, "f(0)"']&
	D'(0)
\end{tikzcd}
...
\end{tikzcd}
\end{equation}
is a pullback in $\smset$; we will freely use \cref{eqn.dir_bund}, i.e.\ the natural isomorphism $D_\pi(X)\cong\bun(X!,\pi)$. One first checks that the square in \cref{eqn.cart1} is a special case of that in \cref{eqn.cart2}, namely for $g\coloneqq 0!$ the unique function $0\to 1$. This establishes the only-if direction. To complete the proof, suppose that \cref{eqn.cart1} is a pullback, take $g\colon X\to X'$, and suppose given a commutative solid-arrow diagram as shown:
\[
...
\end{tikzcd}
\]
We need to show that if the front face is a pullback, then there exists diagonal dotted arrows as shown, making the diagram commute; this is straightforward.
%
%
%
%
